...
export default {
  ...
  provideLinter() {
    ...
    // I know this doesn't follow the project's conventions. It's only an
    // experiment
    (function() {
      const { spawnSync } = require('child_process')
      let node = 'node'
      // node = '/home/asa/.nvm/v0.10.38/bin/node'
      let ret = spawnSync(node, ['-e', 'console.log(process.env.PATH)'])
      console.log('ret', ret)
      if (ret.status === 0) {
        let childPath = ret.stdout.toString().trim()
        const match = childPath === process.env.PATH

        const message = `[eslint experiment] paths ${match ? 'match' : "don't match"}

        ${childPath}
        ${process.env.PATH}
        `
        return atom.notifications.addWarning(message, {dismissable: true} )
      }
